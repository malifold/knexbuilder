...
def runpt(name, epochs=100, lr=1e-3, batch_size=32, use_model=False, epochsaver=1, log_interval=15):
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder('datacreation/model2',
                             transform=data_transforms),
        batch_size=args.batch_size, shuffle=True, num_workers=0)
    val_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder('datacreation/model1',
                             transform=data_transforms),
    ...
    for epoch in range(1, args.epochs + 1):
        train(epoch, verbose=epoch%epochsaver==0)
        #validation()
